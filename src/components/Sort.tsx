import React from 'react';
import { useSelector, useDispatch } from 'react-redux';
import { setSort } from '../redux/slices/filterSlice';
import { RootState } from '../redux/store';


type SortItem = {
  name: string;
   sortProperty:  'rating' | '-rating' | 'price' | '-price' | 'name' | '-name';
}

export const list: SortItem[] = [
  { name: 'популярностю(с)', sortProperty: 'rating' },
  { name: 'популярностю(з)', sortProperty: '-rating' },
  { name: 'ціною(с)', sortProperty: 'price' },
  { name: 'ціною(з)', sortProperty: '-price' },
  { name: 'алфавітом(c)', sortProperty: 'name' },
  { name: 'алфавітом(з)', sortProperty: '-name' },
];

function Sort(): React.ReactElement {
  const dispatch = useDispatch();
  const sortType = useSelector((state: RootState) => state.filters.sortType);
  const sortRef = React.useRef<HTMLDivElement>(null);

  const [open, setOpen] = React.useState(false);

  const onClickListItem = (obj: SortItem) => {
    dispatch(setSort(obj));
    setOpen(false);
  };

  // type PopupClick = React.MouseEvent<HTMLBodyElement> & {
  //   path: Node[]
  // }
  React.useEffect(() => {
    const handelClickOutside = (event: MouseEvent) => {
      const path = event.composedPath ? event.composedPath() : [];
      // Проверяем, есть ли сам ref и корректный path
      if (sortRef.current && !path.includes(sortRef.current)) {
        setOpen(false);
      }
    };

    document.body.addEventListener('click', handelClickOutside);
    return () => {
      document.body.removeEventListener('click', handelClickOutside);
    };
  }, []);

  return (
    <div ref={sortRef} className="sort">
      <div className="sort__label">
        <svg
          width="10"
          height="6"
          viewBox="0 0 10 6"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z"
            fill="#2C2C2C"
          />
        </svg>
        <b>Сортувати за:</b>
        <span onClick={() => setOpen(!open)}>{sortType.name}</span>
      </div>
      {open && (
        <div className="sort__popup">
          <ul>
            {list.map((obj, index) => (
              <li
                key={index}
                onClick={() => onClickListItem(obj)}
                className={
                  sortType.sortProperty === obj.sortProperty ? 'active' : ''
                }
              >
                {obj.name}
              </li>
            ))}
          </ul>
        </div>
      )}
    </div>
  );
}
export default Sort;

// const handelClickOutside = (event) => {
//   if (!event.composedPath().includes(sortRef.current)) {
//     setOpen(false);
//   }
// };
